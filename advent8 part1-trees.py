...
# Advent of code 2022 day 8 part 1        https://adventofcode.com/2022/day/6
# comms device - wood for the trees - which trees are visible from outside the wood
#
import numpy as np                 
...
for i in range(1,len(lines)-1):
    for j in range(1,len(lines[0])-1):
        tree_height = nr[i][j]
        if tree_height>np.amax(nr[i,:j]):
            ans +=1
        elif tree_height>np.amax(nr[i,j+1:]):
            ans +=1
        elif tree_height>np.amax(nr[:i,j]):
            ans +=1
        elif tree_height>np.amax(nr[i+1:,j]):
            ans +=1
            
# all of the trees around the perimeter of the forest are visible from outside so add them
